...
fn main() {
    ...
    for (name, aspect) in [("3:2", 1.5f32), ("16:9", 16.0 / 9.0), ("4:3", 4.0 / 3.0), ("1:1", 1.0)] {
        ...
        // letterbox = (kx*fit/675, ky*aspect*fit/675)
        let lx = kx * fit / 675.0;
        let ly = ky * aspect * fit / 675.0;

        // first visible note
        let (li, line, note) = frame.lines.iter().enumerate()
            .find_map(|(li, l)| l.notes.first().map(|n| (li, l, n))).unwrap();
        // note world pos (renderer): letterbox * T(line_px) * R * T(note_px)
        let ctrl_px = line.position[0] * 675.0;
        let ctrl_py = line.position[1] * 450.0;
        let nx = note.relative[0] * 675.0;
        ...
        // fx world pos: letterbox * T(cx, cy)
        let cx = (line.position[0] + cos * note.relative[0]) * 675.0;
        let cy = (line.position[1] + sin * note.relative[0]) * 450.0;
        let fwx = lx * cx;
        let fwy = ly * cy;
        println!(
            "{name:5} fit={fit:.3} kx={kx:.3} ky={ky:.3} | note world ({nwx:+.3},{nwy:+.3}) fx world ({fwx:+.3},{fwy:+.3}) | Δ({:.3},{:.3})",
            fwx - nwx, fwy - nwy
        );
        let _ = li;
    }
